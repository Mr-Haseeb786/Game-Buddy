...
export default function LibraryGridCard({
  ...
  const imageVariants = {
    normal: { opacity: 1, filter: 'blur(0px) grayscale(0%)' },
    idle: { opacity: 0.15, filter: 'blur(5px) grayscale(100%)' },
    hovered: { opacity: 1, filter: 'blur(0px) grayscale(0%)' },
    dimmed: { opacity: 0.4, filter: 'blur(2px) grayscale(50%)' }
  }

  // 2. The HUD Text Variants
  const textVariants = {
    normal: { opacity: 1, y: 0, textShadow: '0px 0px 0px transparent', letterSpacing: 'normal' },
    idle: {
      opacity: 0.7,
      y: 5,
    ...
    hovered: {
      opacity: 1,
      y: 0,
      // Fixed: Using the variable directly for a solid glow, or fallback to white
      textShadow: '0px 0px 15px var(--app-accent, rgba(255,255,255,0.8))',
  ...
  // 3. The Interactive UI Variants (Buttons, Pills)
  const uiVariants = {
    normal: { opacity: 1, pointerEvents: 'auto' as const },
    idle: { opacity: 0, pointerEvents: 'none' as const },
    hovered: { opacity: 1, pointerEvents: 'auto' as const, transition: { delay: 0.1 } },
    dimmed: { opacity: 0, pointerEvents: 'none' as const }
  }
  ...
  return (
    <motion.div
      layout
      className={`group relative h-[320px] rounded-2xl bg-transparent shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all ${isSettingsOpen || isPillHovered ? 'z-50' : 'z-10 hover:z-40'}`}
    >
      ...
      <div className="absolute inset-0 rounded-2xl overflow-hidden pointer-events-none">
        ...
        <div className="absolute inset-[2px] bg-transparent rounded-[14px] overflow-hidden z-10">
          {!displayImage || imgError ? (
            ...
            >
              <img
                ...
                // Removed the group-hover opacity so Framer Motion controls it cleanly
      ...
      >
        <motion.div variants={uiVariants} animate={cardState}>
          <motion.div
            layout
      ...
      >
        ...
        <div className="w-full">
          <motion.h3
            ...
            animate={cardState}
            transition={{ duration: 0.4, ease: 'easeOut' }}
            // We removed group-hover gradient tricks here because your textShadow handles the glow beautifully now
            className="text-white/95 font-extrabold text-[15px] leading-snug line-clamp-2 break-words drop-shadow-lg shadow-black origin-bottom-left"
          >
            {game.title}
          </motion.h3>
        </div>
